    ...
    <div className="p-4 md:p-6 min-h-full">
      <div className="mb-6">
        <h1 className="text-xl md:text-2xl font-bold text-gray-900 flex items-center">
          <AcademicCapIcon className="h-6 w-6 md:h-8 md:w-8 mr-2 md:mr-3 text-blue-500" />
          Certificates
        </h1>
      ...
      {certificates.length === 0 ? (
        <div className="text-center py-8 md:py-12">
          <AcademicCapIcon className="h-12 w-12 md:h-16 md:w-16 text-gray-400 mx-auto mb-3 md:mb-4" />
          <h3 className="text-base md:text-lg font-medium text-gray-900 mb-2">No certificates found</h3>
        ...
        <>
          ...
          <div className="lg:hidden space-y-4">
            {certificates.map((certificate) => (
              <div key={certificate._id} className="bg-white shadow rounded-lg p-4">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900 font-medium">{certificate.courseTitle}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900 font-mono">{certificate.certificateNumber}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Completion Date:</span>
                    <span className="text-gray-900">{format(new Date(certificate.completionDate), 'MMM dd, yyyy')}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      {showModal && selectedCertificate && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-4 md:top-20 mx-auto p-3 md:p-5 border w-11/12 max-w-md shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
            <div className="mt-3">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-gray-900 break-words">{selectedCertificate.participant.name}</p>
                  <p className="text-sm text-gray-500 break-words">{selectedCertificate.participant.email}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">Course:</label>
                  <p className="text-sm text-gray-900 break-words">{selectedCertificate.courseTitle}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">Certificate Number:</label>
                  <p className="text-sm text-gray-900 break-words">{selectedCertificate.certificateNumber}</p>
                </div>
                <div>
              ...
              <div className="flex justify-end space-x-3 mt-6">
                <button
                  ...
                  className="px-3 md:px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200"
                >
                  Close
                </button>
              </div>
