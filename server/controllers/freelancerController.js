...
const addComplianceDocument = async (req, res) => {
  try {
    ...
    if (req.files && req.files.file && req.files.file[0]) {
      fileUrl = '/uploads/freelancers/' + req.files.file[0].filename;
...
const addWorkHistory = async (req, res) => {
  try {
    const { assignment, startDate, endDate, hours, rate, totalAmount, status, notes } = req.body;
    ...
    const workEntry = {
      ...
      hours: hours || 0,
      rate: rate || 0,
      totalAmount: totalAmount || (hours * rate) || 0,
...
const getExpiringCompliance = async (req, res) => {
  try {
    ...
    const freelancers = await Freelancer.find({
      'complianceDocuments.expiryDate': { $lte: thirtyDaysFromNow }
    });
